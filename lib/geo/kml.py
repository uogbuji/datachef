...
from pyparsing import Word, alphas, alphanums, nums, Literal, restOfLine, OneOrMore, \
    empty, Suppress, replaceWith, Group
...
placemark_set = ( pm_start 
               ...
               + SkipTo(ed_start) + Group(SkipTo(ed_end) + ed_end)("ext_data")
...
def gen_record(text):
    for data, startloc, endloc in placemark_set.scanString(text):
        yield (data["name"], data["description"], data["polygon"], data["ext_data"][0]+'</ExtendedData>')
